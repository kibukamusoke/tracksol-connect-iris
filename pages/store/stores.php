...
<div class="collapse navbar-collapse">
    <ul class="nav navbar-nav navbar-right">
        <li>
            <a href="/istore" class="btn-rotate">
                <i class="ti-plus"></i>
                <span class="notification">Add New Store</span>
            </a>
                ...
                <div class="card">
                    <div class="card-content">
                        ...
                        <table id="bootstrap-table" class="table">
                            ...
                            <?php
                            $query = $__DB->select('store', '*', 'status = 1', 'store_name ASC');
                            while ($result = $__DB->fetch_assoc($query)) {
                                $parentQuery = $__DB->select('store', 'store_name', "store_id = {$result['parent_id']}");
                                $parent = $__DB->fetch_assoc($parentQuery);
                                echo '<tr><td></td>';
                                echo '<td>' . $result['store_id'] . '</td>';
                                echo '<td>' . $result['store_code'] . '</td>';
                                echo '<td>' . $result['store_name'] . '</td>';
                                echo '<td>' . (!empty($parent) ? $parent['store_name'] : '') . '</td>';
                                echo '<td></td>';
                                echo '</tr>';
                            }
                            ?>
    ...
    $().ready(function () {
        window.operateEvents = {
            ...
            'click .edit': function (e, value, row, index) {
                ...
                window.location.href = "/istore/" + row.id + "/modify";
            ...
            'click .remove': function (e, value, row, index) {
                console.log(row);
                $.ajax({
                    url: '/internal/deleteStore',
                    //dataType: 'json',
                    type: 'POST',
                    contentType: 'application/json',
                    data: JSON.stringify({"store_id": row.id}),
                    processData: false,
                    /*headers: {
